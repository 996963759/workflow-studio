...
这是本地单机版 / 私有化版工作流平台雏形。当前前端运行逻辑已支持变量传递和模拟执行；后端已提供 FastAPI、SQLite 工作流 CRUD、工作流结构校验、运行历史接口、本地知识库检索、DeepSeek / OpenAI 大模型节点最小真实调用、本机 HTTP 工具调用和本地账号隔离。当前还没有知识库向量索引和外网工具白名单管理。
