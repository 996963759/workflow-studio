...
- 顶部支持编辑当前工作流名称
- 顶部显示后端在线 / 离线状态
- 管理中心系统概览、运行历史和版本审计已拆成独立前端组件，方便后续维护
...
- 后端会保存工作流归档 / 恢复状态
- 后端会为工作流创建、更新、手动保存、恢复和发布生成版本快照
- 右侧“版本与审计”面板支持查看发布状态，并把当前已同步工作流发布为稳定版本
- 后端提供团队空间级审计日志，记录工作流、成员、模型配置和运行入队等关键操作
- 右侧“版本与审计”面板支持查看版本历史、保存版本、恢复版本、对比两个版本和查看当前工作流审计记录
...
- 右侧管理中心按节点、系统、团队、模型、知识库、运维和 JSON 分区组织管理能力
- 系统概览汇总当前团队空间的数据库、队列、模型、知识库、成员、任务、运行健康、估算成本和最近审计状态
- 系统概览展示登录有效期、邀请码有效期、Worker 数、RAG 开关和允许访问来源，不暴露 API Key
- 后端单元测试、请求日志、环境变量样例和 Docker Compose 部署文件
- 本地账号注册 / 登录，工作流、运行历史和知识库按账号隔离
...
- 运行历史和系统概览展示估算成本单位、计费节点数和 provider 成本构成；该值用于本地演示和趋势判断，不等同于真实云厂商账单
- 知识库上传后自动写入本地向量索引，检索时混合关键词和相似度
- 知识检索节点可切换为 PaiSmart 外部 RAG 服务，并在节点配置里提示团队空间 PaiSmart 状态，支持地址、Token 配置、连接诊断和检索预览
