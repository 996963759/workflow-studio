...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/admin/overview` | 当前团队空间的系统概览，包括数据库、队列、模型、知识库、成员、任务、运行健康、估算成本、最近审计和安全设置摘要 |
...
模型配置按团队空间隔离。前端只展示 API Key / Token 掩码，后端运行大模型、多模态和 PaiSmart RAG 节点时优先使用当前团队空间配置。
...
| GET | `/api/model-configs/aliyun` | 查看当前空间阿里云百炼配置，不返回完整 Key |
| PUT | `/api/model-configs/aliyun` | 保存当前空间阿里云百炼配置 |
| POST | `/api/model-configs/aliyun/test` | 检查当前空间是否已有可用配置 |
| GET | `/api/model-configs/paismart` | 查看当前空间 PaiSmart RAG 配置，不返回完整 Token |
| PUT | `/api/model-configs/paismart` | 保存当前空间 PaiSmart RAG 配置 |
| POST | `/api/model-configs/paismart/test` | 检查当前空间是否已有可用配置 |
| POST | `/api/rag/paismart/preview` | 使用当前空间 PaiSmart 配置预览检索结果 |
| POST | `/api/rag/paismart/diagnose` | 使用当前空间 PaiSmart 配置诊断连接状态 |
...
| GET | `/api/workflows/{workflow_id}/versions/diff?base_version_id={id}&target_version_id={id}` | 对比两个工作流版本，返回新增、删除和变更项 |
| POST | `/api/workflows/{workflow_id}/versions/{version_id}/restore` | 恢复到指定版本，并生成新的恢复快照 |
| POST | `/api/workflows/{workflow_id}/publish` | 发布当前已同步工作流，并生成已发布版本快照 |

创建和更新工作流时，后端会自动生成版本快照。恢复版本不会覆盖历史版本，而是把恢复后的工作流保存为新的版本。发布工作流会把当前后端内容标记为稳定发布版；后续再次修改会把发布状态变为“发布后有改动”。版本对比会比较工作流名称、定义版本、归档状态、节点和连线。
...
运行结果中的每个节点步骤会返回 `input`、`output`、`kind`、`provider`、`error`、`duration_ms` 和 `attempt_count`，方便前端展示节点级输入输出、耗时、失败原因和重试次数。持久化运行记录还会返回 `cost_summary`，包含估算成本单位、计费节点数和 provider 构成；该值用于本地演示和趋势判断，不等同于真实云厂商账单。
...
| DELETE | `/api/run-jobs?workflow_id={id}` | 清理某个工作流已完成、失败和已取消的任务 |
| GET | `/api/run-jobs/{job_id}` | 查看任务状态和完成后的 `run_id` |
| POST | `/api/run-jobs/{job_id}/cancel` | 取消仍在排队中的任务 |
| POST | `/api/run-jobs/{job_id}/retry` | 将失败任务重新入队，并重新发布到当前队列后端 |
