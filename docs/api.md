...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/admin/overview` | 当前团队空间的系统概览，包括数据库、队列、模型、知识库、成员、任务、运行健康、最近审计和安全设置摘要 |
...
运行结果中的每个节点步骤会返回 `input`、`output`、`provider`、`error`、`duration_ms` 和 `attempt_count`，方便前端展示节点级输入输出、耗时、失败原因和重试次数。
