...
9. 打开“评测”，创建一个轻量评测集，批量运行当前工作流并查看通过率。
...
“管理中心 -> 评测”提供第一版轻量评测系统，用来快速判断工作流输出是否符合预期。
...
2. 打开“评测”。
3. 创建评测集，填写评测集名称和描述。
4. 添加样例，每条样例包含输入、期望输出说明和期望关键词。
5. 点击“保存”。
6. 点击“运行评测”，系统会批量运行当前工作流。
7. 查看通过率、最近通过率趋势、失败样例、缺失关键词和每条样例对应的输出。
8. 勾选“只看未通过”，快速定位需要修 prompt 或节点配置的样例。

当前判定规则很轻：只检查工作流最终输出是否包含所有期望关键词。这个版本适合做回归对比和基本质量门禁，不等同于人工评审或 LLM-as-judge。
