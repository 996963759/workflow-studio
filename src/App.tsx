...
const workflowTemplates: WorkflowTemplate[] = [
  ...
  {
    id: 'multimodal-campaign-package',
    name: '短视频口播与配图生成',
    description: '大模型先拆出结构化素材，再同时生成口播音频和营销配图，最后汇总成可发布素材包。',
    input: '为咖啡店新品燕麦拿铁生成一套短视频素材：包含 20 秒口播、封面配图提示词和发布文案，语气温暖，面向上班族。',
    nodes: [
      {
        id: 'input-1',
        type: 'workflow',
        ...
        data: {
          ...
          sampleInput: '为咖啡店新品燕麦拿铁生成一套短视频素材：包含 20 秒口播、封面配图提示词和发布文案，语气温暖，面向上班族。',
      ...
      },
      {
        ...
        data: {
          ...
          prompt:
            '根据需求输出严格 JSON，字段必须包含 title、script、image_prompt、caption、publish_checklist。script 控制在 120 个汉字以内，适合直接 TTS 朗读；image_prompt 要适合中文商品营销海报或短视频封面；publish_checklist 是 3 条字符串数组。\n\nJSON 示例：{"title":"标题","script":"口播文案","image_prompt":"图片生成提示词","caption":"发布文案","publish_checklist":["检查口播节奏","确认图片不含违规元素","发布前补充商品链接"]}\n\n需求：{{campaign_request}}',
      ...
      },
      {
        ...
        data: {
          ...
          ...nodeMeta.output.defaults,
          label: '短视频素材包',
          description: '返回可复制使用的文案、音频结果和图片结果。',
    ...
    edges: [
      { id: 'e-input-llm', source: 'input-1', target: 'llm-1', animated: true },
      { id: 'e-llm-title', source: 'llm-1', target: 'json-title' },
      { id: 'e-llm-script', source: 'llm-1', target: 'json-script' },
      { id: 'e-llm-image', source: 'llm-1', target: 'json-image' },
      { id: 'e-llm-caption', source: 'llm-1', target: 'json-caption' },
      { id: 'e-script-tts', source: 'json-script', target: 'tts-1', animated: true },
      { id: 'e-image-generate', source: 'json-image', target: 'image-1', animated: true },
      { id: 'e-title-aggregate', source: 'json-title', target: 'aggregate-1' },
      { id: 'e-tts-aggregate', source: 'tts-1', target: 'aggregate-1' },
      { id: 'e-image-aggregate', source: 'image-1', target: 'aggregate-1' },
      { id: 'e-caption-aggregate', source: 'json-caption', target: 'aggregate-1' },
